...
\chapter{Expanding the Model}
	...
	\section{Simultaneously Filling and Draining}
		...
		\assignment{
			Adapt the model to stop when it reaches equilibrium not after a specified time as we have been doing before. Use and inflow rate of 30 cm$^3$/s and a drainage coefficient ($k$) of 0.15.
		}
	...
	\assignment{
		...
		Adapt the model so that the water input stops after 50 seconds. Use a fill rate ($Q_{in}$) of 30 cm$^3$/s for the initial inflow rate, and allow the model to drain using the calibrated $k$ coefficient.
	}
	...
	\assignment{
		...
		Change the model so that the water input decreases linearly over time, with an initial inflow rate of 30 cm$^3$/s and a change of -0.1 cm$^3$/s in every second. Note: Watch out for negative inflows.
	...
	\assignment{
		...
		Change the model so that the water input alternatingly turns on at 30 cm$^3$/s for 20 seconds and then turns off for 20 seconds.
		Allow the model to drain using the calibrated $k$ value.
	...
	\assignment{
		...
		\textit{Have the model print out each maximum and minimum height. Determine if the inflow ($Q_{in}$) and height ($h$) are in phase.}
	}
...
\chapter{Solving Differential Equations}
	...
	\begin{tikzpicture}[remember picture,overlay,shift={(current page.east)}]
		\node[anchor=north east,xshift=-1cm](myImage){
			\includegraphics[height=5cm]{constant-slope-line.png} 
		};
		\node[inner sep=0pt,text width=0.5\linewidth] (myCaption) [below=of myImage]
		{\captionof{figure}{\label{constSlopeLine} A function with a constant slope of $\frac{1}{2}$. }};
	\end{tikzpicture}

	A function with a constant slope everywhere must be a straight line. 
	
	Of course, there are many, many straight lines with the same slope. We need to know a single point to be able to pick a specific line. 
